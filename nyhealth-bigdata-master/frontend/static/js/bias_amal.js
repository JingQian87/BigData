...
//import new_york_data csv
//import new_england_data from '../data/a5_newengland_cdc_general_filtered.csv'



//sources: http://bl.ocks.org/diethardsteiner/3287802
const new_york_dashboard = function (d3, topojson) {
    // Data
    var data = d3.dsv(",", "../static/data/birth-avg2white-tukey.csv", function(d) {
          return {
            county: d.County, 
            white: d.White,
            black: d['Black/African American'],
            otherRace: d['Other Race'],
            multiracial: d['Multi-racial']};})
    .then(function(data) {
        console.log(data);
    });
    ...
    d3.select("#inputYearNE")
        .on("change", function () {
            ...
            d3.select("#new_york_bias_map").selectAll("*").remove();
            d3.select("#new_york_gender_bias").selectAll("*").remove();
            d3.select("#new_york_race_bias").selectAll("*").remove();
            new_york_map(data, chosen_year);
...
async function new_york_map(data, year) {
    //US
    function filter_states(new_result) {
        const states = ["36"];
    ...
    var us_map = d3.json('../static/data/10m.json',function(error,us){
        if (error) return console.error(error);
        console.log(us); 
    }).then(function(us_map) {
        console.log(us_map);
        const map_new_york = filter_states(us_map);
    });

    //const map_new_york = filter_states(us_map);
    // States
    const states = new Map(map_new_york.objects.states.geometries.map(d => [d.id, d.properties]));
    // Format
    const format = d => `${d}%`;
    // Color
    ...
    var new_york = topojson.feature(map_new_york, map_new_york.objects.counties);

    var projection = d3.geoIdentity().fitExtent([[20, 20], [940, 580]], new_york);
    ...
    const svg = d3.select("#new_york_map")
    ...
    svg.append("text")
        ...
        .style("fill", "white")
        .text("Overdose Deaths in New york");
    ...
    svg.append("path")
        .attr("class", "ne_mesh")
        .datum(topojson.mesh(map_new_york, map_new_york.objects.states, (a, b) => a !== b))
        .attr("fill", "none")
        .attr("stroke", "white")
...
function gender_barchart(data, county, year) {
    ...
    var svg = d3.select("#new_york_gender")
...
function opioid_type_barchart(data, county, year) {
    ...
    var svg = d3.select("#new_york_opioid_type")
        .attr("width", width + margin.left + margin.right)
        .attr("height", height + margin.top + margin.bottom)
        .append("g")
        .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
...
function update_graphs(d) {
    d3.select("#new_york_gender").selectAll("*").remove();
    d3.select("#new_york_opioid_type").selectAll("*").remove();
...
new_york_dashboard(d3, topojson);
